...
class TwentyFortyEight:
    ...
    def __init__(self, grid_height, grid_width):
        ...
        lst_right = ([[i_idx,self.grid_width-1] for i_idx in range(0,self.grid_width)])
        lst_left = ([[i_idx,0] for i_idx in range(0,self.grid_width)])
        lst_down = ([[self.grid_height-1,i_idx] for i_idx in range(0,self.grid_height)])
        self.directions = {UP: lst_up,
                           RIGHT : lst_right,
                           LEFT : lst_left,
                           DOWN : lst_down,
                           }
        self.dashboard = [[0 for col in range(self.grid_width)] for row in range(self.grid_height)]
    ...
    def move(self, direction):
        ...
        temp_dashboard = [[random.randrange(0,8,2) for col in range(4)] for row in range(4)]
